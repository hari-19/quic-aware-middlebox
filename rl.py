...
class RateLimitedSniffer(threading.Thread):
    def __init__(self, token_buckets, quic_IP_tuples, input_iface, output_iface, logger, filter=None):
        super().__init__()
...
def print_token_bucket(token_buckets):
    with open("logs/{}_mappings.txt".format(CLIENT_FACING_IFACE), "w") as f:
...
if __name__ == "__main__":
    ...
    in_sniffer = RateLimitedSniffer(token_buckets, quic_ip_tuples, filter= "ip and (udp) and not host " + AGENT_IP,input_iface=CLIENT_FACING_IFACE, output_iface=SERVER_FACING_IFACE, logger=logger_client)
    in_sniffer.start()

    out_sniffer = RateLimitedSniffer(token_buckets, quic_ip_tuples, filter="ip and (udp or tcp) and not host " + AGENT_IP, input_iface=SERVER_FACING_IFACE, output_iface=CLIENT_FACING_IFACE, logger=logger_server)
    out_sniffer.start()
